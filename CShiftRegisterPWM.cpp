/*
CShiftRegisterPWM.cpp - ShiftRegisterPWM.h - Library for Arduino to PWM many outputs using shift registers
...
CShiftRegisterPWM::CShiftRegisterPWM(int timerInUse, bool noSPI, int latchPin, int dataPin, int clockPin) :  // Constants are set in initializer list
...
CShiftRegisterPWM::~CShiftRegisterPWM() {
	if(m_PWMValues>0){
		free( m_PWMValues );
	}
}

bool CShiftRegisterPWM::IsValidPin(int pin){
...
void CShiftRegisterPWM::SetOne(int pin, unsigned char value){
	if(IsValidPin(pin) ){
		m_PWMValues[pin]=value;
	}
}

void CShiftRegisterPWM::SetAll(unsigned char value){
	for(int k=0 ; k<(m_amountOfOutputs);k++){
		m_PWMValues[k]=value;
	}
}

void CShiftRegisterPWM::SetGroupOf2(int group, unsigned char v0,unsigned char v1, int offset){
...
void CShiftRegisterPWM::SetGroupOf3(int group, unsigned char v0,unsigned char v1,unsigned char v2, int offset){
...
void CShiftRegisterPWM::SetGroupOf4(int group, unsigned char v0,unsigned char v1,unsigned char v2,unsigned char v3, int offset){
...
void CShiftRegisterPWM::SetGroupOf5(int group, unsigned char v0,unsigned char v1,unsigned char v2,unsigned char v3,unsigned char v4, int offset){
...
void CShiftRegisterPWM::SetRGB(int led, unsigned char r,unsigned char g,unsigned char b, int offset){
...
void CShiftRegisterPWM::SetAllRGB(unsigned char r,unsigned char g,unsigned char b){
...
void CShiftRegisterPWM::SetHSV(int led, unsigned int hue, unsigned int sat, unsigned int val, int offset){
	unsigned char r,g,b;
...
void CShiftRegisterPWM::SetAllHSV(unsigned int hue, unsigned int sat, unsigned int val){
	// Set the first LED
...
void CShiftRegisterPWM::OneByOneSlow(void){
	OneByOne_core(1024/m_maxBrightness);
}

void CShiftRegisterPWM::OneByOneFast(void){
	OneByOne_core(1);
}

void CShiftRegisterPWM::OneByOne_core(int delaytime){
	int pin,brightness;
	SetAll(0);
...
void CShiftRegisterPWM::SetAmountOfRegisters(unsigned char newAmount){
...
void CShiftRegisterPWM::SetPinGrouping(int grouping){
...
bool CShiftRegisterPWM::LoadNotTooHigh(void){
	...
	float interruptFrequency = m_ledFrequency * ((float) m_maxBrightness + 1);
...
void CShiftRegisterPWM::Start(float ledFrequency, unsigned char maxBrightness){
...
void CShiftRegisterPWM::PFCUpdate(float ledFrequency)
{
    m_ledFrequency = ledFrequency;
    
    if(m_timer==1){
        OCR1A = round((float) F_CPU/(m_ledFrequency*((float) m_maxBrightness+1)))-1;
    }
#if defined(USBCON)
    else if(m_timer==3){
        OCR3A = round((float) F_CPU/(m_ledFrequency*((float) m_maxBrightness+1)))-1;
    }
#else
    else if(m_timer==2){
        InitTimer2();
...
void CShiftRegisterPWM::InitTimer1(void){
...
#if defined(OCR2A)
void CShiftRegisterPWM::InitTimer2(void){
	...
	*  See table 15-5 in the datasheet. */
	int compare_value =  round((float) F_CPU/(m_ledFrequency*((float) m_maxBrightness+1))-1);
	if(compare_value <= 255){
		...
		* So the value we want for OCR2A is: timer clock frequency/(LED frequency * number of bightness levels)-1 */
		OCR2A = round(   (  (float) F_CPU / (float) m_prescaler ) /  (m_ledFrequency*( (float) m_maxBrightness+1) ) -1);
		/* Finally enable the timer interrupt, see datasheet  15.11.8) */
...
void CShiftRegisterPWM::InitTimer3(void){
	/*
...
void CShiftRegisterPWM::PrintInterruptLoad(void){
	//This function prints information on the interrupt settings for ShiftRegisterPWM
	...
	Serial.print(F("PWM frequency: ")); Serial.print(interrupt_frequency/(m_maxBrightness+1)); Serial.println(F(" Hz"));
		...
		if(m_timer==1){
			...
			Serial.println(F("add '#define ShiftRegisterPWM_USE_TIMER3' before '#include <ShiftRegisterPWM.h>' to switch to timer 3."));
		...
		if(m_timer==1){
			...
			Serial.println(F("add '#define ShiftRegisterPWM_USE_TIMER2' before '#include <ShiftRegisterPWM.h>' to switch to timer 2."));
			Serial.print(F("OCR1A: ")); Serial.println(OCR1A, DEC);
			Serial.print(F("Prescaler: ")); Serial.println(m_prescaler);
